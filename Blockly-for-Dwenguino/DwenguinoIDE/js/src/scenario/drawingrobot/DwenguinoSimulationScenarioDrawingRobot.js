...
//   // prevent the list being filled by the same value constdsantly
//   if( (this.motor.settings.stepsL[this.motor.settings.stepsL.length-1] !== Math.round(dwenguinoState.motorSteps[0],0)) && (this.motor.settings.stepsR[this.motor.settings.stepsR.length-1] !== Math.round(dwenguinoState.motorSteps[1],0))){
//     this.motor.settings.stepsL.push(Math.round(dwenguinoState.motorSteps[0],0));
//     this.motor.settings.stepsR.push(Math.round(dwenguinoState.motorSteps[1],0));
//     // if(this.motor.settings.stepsL.length > 0 || this.motor.settings.stepsR.length > 0){
//       this.stylus.drawing.angles.push(dwenguinoState.servoAngles[0]);
//       this.stylus.drawing.colors.push(dwenguinoState.color);
//     // }

//     // console.log("received (" + dwenguinoState.motorSteps[0] + ";" + dwenguinoState.motorSteps[1] + ")");
//   }

  
//   var x1 = Math.round(this.stylus.position.x,0);
//   var y1 = Math.round(this.stylus.position.y,0);
//   var length = this.line.lengthBase;
//   // console.log("p2: " + this.motor.settings.stepsL[0] + " - " + this.motor.settings.stepsR[0]);
//   var p2 = DwenguinoSimulationScenarioDrawingRobot.prototype.calculatePoint(this.motor.settings.stepsL[0]+length, this.motor.position.xL, this.motor.position.y, this.motor.settings.stepsR[0]+length, this.motor.position.xR, this.motor.position.y);
//   var x2 = Math.round(p2[0],0);
//   var y2 = Math.round(p2[1],0);
//   // console.log(x1,y1);
//   // console.log(x2,y2);

//   if(x1 === x2 && y1 === y2){ //if start position == next position -> skip to next step
//     this.motor.settings.stepsL.shift();
//     this.motor.settings.stepsR.shift();
//     DwenguinoSimulationScenarioDrawingRobot.prototype.updateServoAngles(this.stylus.drawing.angles.shift(), this);
//     this.stylus.drawing.color = this.stylus.drawing.colors.shift();
//   } else {
//     if (this.motor.settings.steps.length === 0 ){ // no intermediate steps left to move from start position to next position
//       // console.log("if");
//       if( (x1 !== x2) || (y1 !== y2)){ //There is a new position -> calculate the intermediate steps to go to this new position 
//         this.motor.settings.steps = DwenguinoSimulationScenarioDrawingRobot.prototype.bresenham(x1,y1,x2,y2,dwenguinoState);
//         // console.log("start with: " + this.motor.settings.stepsL[0] + " - " +this.motor.settings.stepsR[0]);
//         this.motor.settings.stepsL.shift();
//         this.motor.settings.stepsR.shift();
//         DwenguinoSimulationScenarioDrawingRobot.prototype.updateServoAngles(this.stylus.drawing.angles.shift(), this);
//         this.stylus.drawing.color = this.stylus.drawing.colors.shift();
//         // console.log("ifif");
//       }
//     } else { // Busy with intermediate steps to got from start to next position
//       // console.log("else");
//       var positie = this.motor.settings.steps.shift();

//       var newLength = DwenguinoSimulationScenarioDrawingRobot.prototype.getLength(positie[0],positie[1]);
  
//       this.motor.settings.currentStepsL = newLength[0];
//       this.motor.settings.currentStepsR = newLength[1];
  
//       this.stylus.prevPosition.x = this.stylus.position.x;
//       this.stylus.prevPosition.y = this.stylus.position.y;
//       this.line.lengthL = newLength[0];
//       this.line.lengthR = newLength[1];
//       this.stylus.position.x = positie[0];
//       this.stylus.position.y = positie[1];
...
//   // prevent the list being filled by the same value/drawing constantly
//   if( (this.motor.settings.stepsL[this.motor.settings.stepsL.length-1] !== Math.round(dwenguinoState.motorSteps[0],0)) && (this.motor.settings.stepsR[this.motor.settings.stepsR.length-1] !== Math.round(dwenguinoState.motorSteps[1],0))){
//     this.motor.settings.stepsL.push(Math.round(dwenguinoState.motorSteps[0],0));
//     this.motor.settings.stepsR.push(Math.round(dwenguinoState.motorSteps[1],0));
//   }

//   var diff1 = Math.abs(this.motor.settings.currentStepsL - this.motor.settings.stepsL[0]);
//   var diff2 = Math.abs(this.motor.settings.currentStepsR - this.motor.settings.stepsR[0]);
//   var maxDifference = Math.max(diff1,diff2);
  

//   var l = 0;
//   var r = 0;
//   if(maxDifference !== 0){
//     l = diff1 / maxDifference;
//     r = diff2 / maxDifference;
//   }
  
  
//   // console.log("steps:" + maxDifference +"  change L by " + l + "  en R by "+ r);
//   if(this.motor.settings.currentStepsL > this.motor.settings.stepsL[0]){
//     // this.motor.settings.currentStepsL--;
//     this.motor.settings.currentStepsL -= l
//   } else if (this.motor.settings.currentStepsL < this.motor.settings.stepsL[0]) {
//     // this.motor.settings.currentStepsL++;
//     this.motor.settings.currentStepsL+=l;
//   }

//   if(this.motor.settings.currentStepsR > this.motor.settings.stepsR[0]){
//     // this.motor.settings.currentStepsR--;
//     this.motor.settings.currentStepsR-=r;
//   } else if (this.motor.settings.currentStepsR < this.motor.settings.stepsR[0]) {
//     // this.motor.settings.currentStepsR++;
//     this.motor.settings.currentStepsR+=r;
//   }

//   // Remove the item from the list when it's finished drawing
//   if(Math.round(this.motor.settings.currentStepsL,0) === this.motor.settings.stepsL[0] && Math.round(this.motor.settings.currentStepsR,0) === this.motor.settings.stepsR[0]){
//     this.motor.settings.stepsL.shift();
//     this.motor.settings.stepsR.shift();
//     l 
//   }
...
//   this.line.lengthL = this.line.lengthBase + this.motor.settings.currentStepsL;
//   this.line.lengthR = this.line.lengthBase + this.motor.settings.currentStepsR;

//   var points = DwenguinoSimulationScenarioDrawingRobot.prototype.calculatePoint(this.line.lengthL, this.motor.position.xL, this.motor.position.y, this.line.lengthR, this.motor.position.xR, this.motor.position.y);

//   this.stylus.position.x = points[0];
//   this.stylus.position.y = points[1];
//   return dwenguinoState;
// };
...
// DwenguinoSimulationScenarioDrawingRobot.prototype.bresenham = function(x1, y1, x2, y2){
